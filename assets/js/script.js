$(document).ready(function () {
  ...
  for (var i = 0; i < hashParams.length; i++) {
    ...
    if (fieldsValue[0] === 'first_name') {
      document.getElementById('entry_1951006374').value = decodeURIComponent(fieldsValue[1])
    } else if (fieldsValue[0] === 'last_name') {
      document.getElementById('entry_365272651').value = decodeURIComponent(fieldsValue[1])
    } else if (fieldsValue[0] === 'email') {
      document.getElementById('entry_2121602646').value = decodeURIComponent(fieldsValue[1])
    }
  }
  // NRIC number validation if its a proper number
  $('#entry_828261091').blur(function () {
    var icNum = document.getElementById('entry_828261091').value
    icNum = icNum.toUpperCase()
    document.getElementById('entry_828261091').value = icNum
    console.log(icNum)
    var icArray = []
    ...
    if (icNum.length === 9) {
      ...
      if (checkSum === icArray[8]) {
        ...
        $('#entry_828261091').removeClass('error')
        $('#entry_828261091').addClass('valid')
      } else {
        console.log('Invalid NRIC Number')
        $('#entry_828261091').addClass('error')
        $('#entry_828261091').removeClass('valid')
      }
    }
  })

  // Validating of IMEI number
  $('#entry_744152826').blur(function () {
    var imei = document.getElementById('entry_744152826').value
    if (imei.length === 15) {
      var imeiDigit = 0
      var imeiWeight = 0
      var bEven = true
      ...
      if (lastImeiNum === checkImeiSum) {
        ...
        $('#entry_744152826').removeClass('error')
        $('#entry_744152826').addClass('valid')
      } else {
        console.log('IMEI is invalid')
        $('#entry_744152826').addClass('error')
        $('#entry_744152826').removeClass('valid')
      }
    }
  })
  ...
  var autocomplete = new google.maps.places.Autocomplete(
    (document.getElementById('entry_1106704322')),
    {types: ['geocode']}
  )
  ...
  function fillInAddress () {
    ...
    document.getElementById('entry_1106704322').value = address1
    var postalCode = place.address_components[5].long_name
    document.getElementById('entry_1907900927').value = postalCode
  }
})
